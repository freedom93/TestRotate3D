...
public class MainActivity extends Activity {
	...
	private View.OnClickListener listener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.main_last:	// 向左
				layoutmain.startAnimation(lQuest1Animation);	// 向左旋转180°
				initNext();
				layoutnext.startAnimation(lQuest2Animation);	// 向左旋转180°
				break;
			case R.id.main_next:	// 向右
				layoutmain.startAnimation(rQuest1Animation);	// 向左旋转180°
				initNext();
				layoutnext.startAnimation(rQuest2Animation);	// 向右旋转180°
				break;
			case R.id.next_last:
	...
	public void initAnimation() {
		...
		int duration = 1000;
		//向左的question1旋转方向（从0度转到-180，参考系为水平方向为0度）
		lQuest1Animation = new Rotate3D(0, -180, mCenterX, mCenterY);	
		lQuest1Animation.setFillAfter(true);
		lQuest1Animation.setDuration(duration);

		//向左的question2旋转方向（从180度转到0，参考系为水平方向为0度）
		lQuest2Animation = new Rotate3D(180, 0, mCenterX, mCenterY);		
		lQuest2Animation.setFillAfter(true);
		lQuest2Animation.setDuration(duration);

	    //向右的question1旋转方向（从0度转到180，参考系为水平方向为0度）
		rQuest1Animation = new Rotate3D(0, 180, mCenterX, mCenterY);		
		rQuest1Animation.setFillAfter(true);
		rQuest1Animation.setDuration(duration);

		//向右的question2旋转方向（从-180度转到0，参考系为水平方向为0度）
		rQuest2Animation = new Rotate3D(-180, 0, mCenterX, mCenterY);	
